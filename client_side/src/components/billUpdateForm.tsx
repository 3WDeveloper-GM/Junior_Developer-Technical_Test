...
export const UpdateBillForm: FC<BillFormProps> = (props) => {
  const { previousData } = props;

  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      billID: previousData?.idFactura,
      details: {
  ...
  const submit = async (data: z.infer<typeof FormSchema>) => {
    const bill: Omit<Bill, "proveedor"> = {
      numeroRegistro: previousData?.numeroRegistro,
      idFactura: data.billID,
      fechaEmision: DateTime.fromISO(data.emissionDate).toISO({suppressMilliseconds:true}),
